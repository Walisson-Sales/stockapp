  ...
  const handleSave = async () => {
    ...
    try {
      if (produtoParaEditar) {
        // Na edição, o backend pode não aceitar atualizar o estoque aninhado diretamente na mesma rota dependendo da regra,
        // mas vamos tentar enviar conforme a tipagem permite.
        // Omitimos 'estoque' se o backend não atualizar estoque na rota de produto (comum em sistemas reais)
      ...
      <DialogContent>
        <Box display="flex" flexDirection="column" gap={2} mt={1}>
          ...
          <Box display="flex" gap={2}>
             {/* SELECT DE CATEGORIA IMPORTANTE */}
            <FormControl fullWidth>
              <InputLabel>Categoria</InputLabel>
              <Select
                name="idCategoria"
                value={formData.idCategoria || ""} // Evita warning de uncontrolled
                label="Categoria"
          ...
          {/* Só mostramos estoque na criação. Na edição, geralmente se usa "Movimentação" */}
          {!produtoParaEditar && (
            <Box display="flex" gap={2}>
